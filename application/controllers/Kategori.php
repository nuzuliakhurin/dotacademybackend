...
class Kategori extends CI_Controller {
	...
	{
		if ($this->session->userdata('logged_in') == TRUE) {
			$this->form_validation->set_rules('ubah_nama_kategori', 'nama_hijab', 'trim|required');

			if ($this->form_validation->run() == TRUE ) {
				if ($this->kategori_model->ubah() == TRUE) {
					$this->session->set_flashdata('notif', 'Ubah data kategori berhasil');
					redirect('kategori/index');
				} else {
					$this->session->set_flashdata('notif', 'Ubah data kategori gagal');
					redirect('kategori/index');
				}
			} else {
				$this->session->set_flashdata('notif', validation_errors());
	...
	public function hapus()
	{
		if ($this->session->userdata('logged_in') == TRUE) {
			...
			if ($this->kategori_model->hapus() ==TRUE) {
				$this->session->set_flashdata('notif', 'Penghapusa data kategori berhasil');
				redirect('kategori/index');
			} else {
				$this->session->set_flashdata('notif', 'Penghapusan data kategori gagal');
				redirect('kategori/index');
			}
		} else {
			redirect('login/index');
